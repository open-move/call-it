...
const checkpointSchema = z.object({ sequenceNumber: z.number().int().nonnegative() })
...
export async function getLatestGraphqlCheckpoint(url: string): Promise<bigint> {
  const data = await graphqlRequest(url, latestCheckpointQuery, {}, latestCheckpointDataSchema)
...
export async function fetchStrategyModuleEventPage(input: {
  ...
  return {
    events: data.events.edges.map((edge) => graphqlEventEdgeToCheckpointEvent(edge)),
...
async function graphqlRequest<T extends z.ZodType>(
  ...
  if (parsed.errors !== undefined && parsed.errors.length > 0) {
    throw new Error(`GraphQL request failed: ${parsed.errors.map((error) => error.message).join("; ")}`)
  }
...
function bigintToSafeNumber(value: bigint): number {
  if (value > BigInt(Number.MAX_SAFE_INTEGER)) {
    throw new Error(`checkpoint ${value.toString()} exceeds Number.MAX_SAFE_INTEGER`)
  }
  return Number(value)
}
