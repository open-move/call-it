...
async function ingestOnce(
  ...
  for (const pipeline of pipelines) {
    ...
    logger.info(
      toLogFields({
        cursor: (await repo.getCursor(pipeline.name))?.toString() ?? null,
...
async function ingestForever(
  ...
  if (config.strategyBackfillOnStart) {
    ...
    // A backfill failure must not block the live ingest streams below — the
    // gRPC tail keeps the projection current and the backfill retries on the
    // next restart.
    try {
      await backfillStrategyPerformanceFromGraphql(config, client, repo);
    } catch (error) {
      logger.error(
        ...
        "strategy GraphQL backfill failed; continuing with live ingest",
      );
    }
  }
  await Promise.all([
...
async function serve(
  ...
  void ingestForever(config, client, repo, pipelines).catch(
    (error: unknown) => {
      logger.error(
        { error: error instanceof Error ? error.message : String(error) },
  ...
  const cursors = Object.fromEntries(
    await Promise.all(
      pipelines.map(async (pipeline) => [
        pipeline,
        (await repo.getCursor(pipeline))?.toString() ?? null,
      ]),
    ),
  );
  logger.info(
