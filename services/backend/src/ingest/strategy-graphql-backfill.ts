...
import { fetchStrategyModuleEventPage, getLatestGraphqlCheckpoint } from "../sui/graphql-events.ts"
...
export async function backfillStrategyPerformanceFromGraphql(
  ...
  logger.info(toLogFields({ target: target.toString() }), "strategy GraphQL backfill start")
...
export async function backfillStrategyPipelineFromGraphql(input: {
  ...
  if (fromCursor >= input.target) {
    const cursor = storedCursor !== null && storedCursor > input.target ? storedCursor : input.target
  ...
  do {
    ...
    if (page.hasNextPage && page.nextCursor === null) {
      throw new Error(`GraphQL returned hasNextPage without an endCursor for ${input.pipeline.name}`)
    }
  ...
  if (liveTip > input.target) {
    await backfillRange(input.client, input.repo, input.pipeline, input.target + 1n, liveTip)
  }
  ...
  logger.info(
    toLogFields({
      count,
      cursor: (await input.repo.getCursor(input.pipeline.name))?.toString() ?? null,
