...
///
/// Deployed/reserve capital lives in the shared Base Vault as `base_shares`;
/// quote only ever exists as a transient local `Coin<Quote>` inside a single
/// function (redeem -> trade/settle -> redeposit). The one standing exception is
/// `pending_deposits`: quote parked by depositors who arrived mid-round, held
/// aside and excluded from NAV until it converts to shares at the next roll.
///
...
public struct Strategy<phantom Quote> has key {
    ...
    /// STRANGLE shares minted for settled pending deposits, escrowed here until
    /// each depositor claims their pro-rata slice. Already part of total supply.
    pending_share_pool: Balance<STRANGLE>,
