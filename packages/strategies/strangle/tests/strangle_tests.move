...
#[test]
fun settle_down_win_returns_payout_to_base_shares() {
    let mut test = begin(ADMIN);
    let env = setup_strategy(&mut test);
    deposit_as(&mut test, &env, USER, DEPOSIT_AMOUNT);
    start_round(&mut test, &env, DOWN_STRIKE, DOWN_QUANTITY, UP_STRIKE, UP_QUANTITY);
...
#[test]
fun settle_inside_strangle_keeps_no_payout() {
    let mut test = begin(ADMIN);
    let env = setup_strategy(&mut test);
    deposit_as(&mut test, &env, USER, DEPOSIT_AMOUNT);
    start_round(&mut test, &env, DOWN_STRIKE, DOWN_QUANTITY, UP_STRIKE, UP_QUANTITY);
...
fun request_settle_claim_returns_fair_pro_rata() {
    ...
    // USER claims fair value at the settled price (~the surviving premium pool).
...
fun stale_sweep_reenrolls_after_grace() {
    ...
    // USER is re-issued fresh STRANGLE shares.
    test.next_tx(USER);
    {
        let shares = test.take_from_sender<Coin<STRANGLE>>();
...
fun invalid_policy_aborts() {
    let _policy = policy::new(PREMIUM_BUDGET_BPS, STRIKE_BAND_BPS, RESERVE_BPS, 0);
}

#[test, expected_failure]
fun invalid_wide_strike_band_policy_aborts() {
    let _policy = policy::new(PREMIUM_BUDGET_BPS, 501, RESERVE_BPS, MAX_LEG_ASK_BPS);
}
